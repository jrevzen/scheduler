...
def getparams(x):
    if x == 1:
        with open('scheduling/maintenance_constraints.json') as f:
            data = json.load(f)
        return data
    elif x == 2:
        with open('scheduling/maintenance_constraints.json') as f:
...
def solve_shift_scheduling(args, sector):
    """Solves the shift scheduling problem."""
    # Data
    data = getparams(sector)
    ...
    ###
    # added argument constraint
    ###
    for i in range(len(args)):
        requests.append(args[i])
    ...
    weekly_cover_demands = [
        ...
        (3, 1, 1),  # Monday   יום ב
        (3, 1, 1),  # Tuesday יום ג
        (3, 1, 1),  # Wednesday יום ד
        (3, 1, 1),  # Thursday    יום ה
        (1, 1, 1),  # Friday  יום ו
        (1, 0, 1),  # Saturday    יום ש
    ]
    ...
    for s in range(1, num_shifts):
        for w in range(num_weeks):
            for d in range(7):
                ...
                # Added constraint for shabat
                if d == 5 or d == 6:
                    excess_cover_penalties = (5, 5, 5)
                else:
                    excess_cover_penalties = (0, 5, 5)
                ###
...
def main(args, sector):
    """Main."""

    # def split(word):
    #     return [char for char in word]

    solution = solve_shift_scheduling(args, sector)


if __name__ == '__main__':
    constraints = [(1, 0, 0, -4), (2, 0, 0, -4)]
